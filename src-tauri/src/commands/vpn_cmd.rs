...
#[tauri::command]
pub async fn vpn_status(state: State<'_, AppState>) -> CmdResult {
    let _ = state.actor()?;
    run_value(move || Ok(state.vpn.status_value(None)))
}

#[tauri::command]
pub async fn vpn_probe(state: State<'_, AppState>) -> CmdResult {
    let _ = state.actor()?;
    run_value(move || Ok(state.vpn.probe()))
}

#[tauri::command]
pub async fn vpn_connect(state: State<'_, AppState>, payload: Option<Value>) -> CmdResult {
...
#[tauri::command]
pub async fn vpn_disconnect(state: State<'_, AppState>) -> CmdResult {
    let actor = state.actor()?;
    run_value(move || futures_now(state.vpn.disconnect(&actor)))
}

#[tauri::command]
pub async fn vpn_diagnose(state: State<'_, AppState>) -> CmdResult {
    let _ = state.actor()?;
